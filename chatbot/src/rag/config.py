...
class QdrantConfig(BaseModel):
    ...
    url: str = Field(default_factory=lambda: os.getenv("QDRANT_URL", ":memory:"), description="Qdrant storage (url or :memory: or /path/to/db)")
    api_key: Optional[str] = Field(default_factory=lambda: os.getenv("QDRANT_API_KEY"), description="Qdrant API key (for cloud)")
    collection_name: str = Field(default="ai-native-book-384", description="Collection name")
    vector_size: int = Field(default=384, description="Vector dimensions (all-MiniLM-L6-v2)")
...
class HuggingFaceConfig(BaseModel):
    ...
    api_key: str = Field(default_factory=lambda: os.getenv("HF_TOKEN", ""), description="Hugging Face API token")
